...
class CenternetDataset(Dataset):
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        if not random:
            ...
            nw = int(iw*scale)
            nh = int(ih*scale)
            dx = (w-nw)//2   # （dx ,dy）指paste贴图的起始位置（左上角）
            dy = (h-nh)//2
            ...
            new_image   = Image.new('RGB', (w,h), (128,128,128))  # 定义一个灰度图，大小是（w，h）即需要改变的大小
            new_image.paste(image, (dx, dy))  # 将变化后的图片贴到灰度图上，作用类似于将原图填充到（w,h）大小
