...
from nets.resnet18dcn import resnet18dcn,resnet18dcn_Decoder,resnet18dcn_Head
from nets.resnet50dcn import resnet50dcn,resnet50dcn_Decoder,resnet50dcn_Head
...
class CenterNet_Resnet18DCN(nn.Module):
    def __init__(self, num_classes = 20, pretrained = False):
        super(CenterNet_Resnet18DCN, self).__init__()
        self.pretrained = pretrained
        # 512,512,3 -> 16,16,512
        self.backbone = resnet18dcn(pretrained = pretrained)
        # 16,16,512 -> 128,128,64
        self.decoder = resnet18dcn_Decoder(512)
        #-----------------------------------------------------------------#
        #   对获取到的特征进行上采样，进行分类预测和回归预测
        #   128, 128, 64 -> 128, 128, 64 -> 128, 128, num_classes
        #                -> 128, 128, 64 -> 128, 128, 2
        #                -> 128, 128, 64 -> 128, 128, 2
        #-----------------------------------------------------------------#
        self.head = resnet18dcn_Head(channel=64, num_classes=num_classes)
...
class CenterNet_Resnet50DCN(nn.Module):
    def __init__(self, num_classes = 20, pretrained = False):
        super(CenterNet_Resnet50DCN, self).__init__()
        self.pretrained = pretrained
        # 512,512,3 -> 16,16,512
        self.backbone = resnet50dcn(pretrained = pretrained)
        # 16,16,512 -> 128,128,64
        self.decoder = resnet50dcn_Decoder(2048)
        #-----------------------------------------------------------------#
        #   对获取到的特征进行上采样，进行分类预测和回归预测
        #   128, 128, 64 -> 128, 128, 64 -> 128, 128, num_classes
        #                -> 128, 128, 64 -> 128, 128, 2
        #                -> 128, 128, 64 -> 128, 128, 2
        #-----------------------------------------------------------------#
        self.head = resnet50dcn_Head(channel=64, num_classes=num_classes)
...
if __name__ == "__main__":
    ...
    model = CenterNet_Resnet18(num_classes=4).to("cpu")
    #summary(model,(3,640,640))

    dummy_input1 = torch.randn(1,3,640,640).cpu()
    input_names = ["input"]
    output_names =  ["outputs"]
    torch.onnx.export(model,dummy_input1,"center_resnet18.onnx",
