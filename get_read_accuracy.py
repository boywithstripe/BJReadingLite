...
import crnn.models.crnn_vgg as crnn_new
...
def parse_arg():
    ...
    parser.add_argument('--checkpoint',default="crnn_file/stn_crnn_model.pth",help="the path to your checkpoints")
    #parser.add_argument('--stn',default=False,help="use stn module or not")
    parser.add_argument('--model',default="crnn_stn",help="crnn|crnn_stn|crnn_vgg")
...
class Crnn(object):
    ...
    def generate(self):
        ...
        elif args.model == "crnn_stn":
            self.model = stn_crnn.get_crnn(self.config).to(device)
        elif args.model == "crnn_vgg":
            self.model = crnn_new.get_crnn(self.config).to(device)
        else:
            raise "no model"
...
class CenterNet(object):
    _defaults = {
        ...
        # --------------------------------------------------------------------------#
         #"model_path"        : 'logs/mobilenetv2_mo_480_640/ep500-loss0.829-val_loss0.714.pth',
        #"model_path"        : './logs/resnet18_768_768/ep300-loss0.686-val_loss0.659.pth',
        #"model_path"         : "./logs/mobilenetv3_768_768/ep500-loss0.745-val_loss0.711.pth",
        "model_path"         : "./logs/mobilenetv2_768_768/ep500-loss0.785-val_loss0.738.pth",
        #"model_path"         : "./logs/ghostnet_new_512_512/ep500-loss0.790-val_loss0.765.pth",
        #"model_path"          : "./logs/resnet50_768_768/ep300-loss0.639-val_loss0.623.pth",
        #"model_path"          : "./logs/shufflenetv2_768_768/ep500-loss0.836-val_loss0.801.pth",
        #"model_path"          : "./logs/ghostnet_768_768/ep500-loss0.798-val_loss0.731.pth",
        #"model_path"          : "./logs/ghostnet_new_512_512/ep500-loss0.790-val_loss0.765.pth",
        #"model_path"           :"./logs/mobilenetv2_mo_768_768/ep300-loss0.944-val_loss0.849.pth",
        #"model_path"          : "./logs/pgmdet_1_512_512/ep100-loss0.861-val_loss0.812.pth",
        #"model_path"          : "./logs/mobilenetv2_centernet_512_512_numclasses_4/ep300-loss0.609-val_loss0.544.pth",
        #"model_path": './logs/resnet18_768_768/ep300-loss0.686-val_loss0.659.pth',  # resnet18dcn_768_768
        #"model_path"         : "./logs/pgmdet_2_768_768/ep500-loss0.785-val_loss0.738.pth",
        ...
        "backbone":"mobilenetv2",
        #"backbone": "shufflenet",
        #"backbone": "ghostnet_new",
        #"backbone": "pgmdet",
        #"backbone":"mobilenetv2_mo",
        #"backbone" : "mobilenetv2_centernet",
        #"backbone" : "pgmdet2",
        # --------------------------------------------------------------------------#
        #   输入图片的大小，设置成32的倍数
        # --------------------------------------------------------------------------#
    ...
    def generate(self):
        ...
        assert self.backbone in ['resnet50', 'hourglass', 'resnet18','resnet18dcn','resnet50dcn','mobilenetv2_mo','mobilenetv3_mo',
                                    "mobilenetv2","mobilenetv3","shufflenet","ghostnet","ghostnet_new","pgmdet","mobilenetv2_centernet","pgmdet2"]
    ...
    #   检测图片
    # ---------------------------------------------------#
    def detect_image(self, image,args,crop=False):
        center = []
        pointer = []
        scale = []
        num_p = []
...
if __name__ == '__main__':
    ...
    test_txt = "./data_bj_num_all/test.txt"
    json_path = "./data_bj_num_all/bj/jsons/"
    image_save_path = "./test_save"
    count_read_sum = 0
    count_correct_005 = 0
    count_correct_010 = 0
    ...
    for imagedata in data:
        ...
        read_models = dict()
        range_labels = dict()
        imagefile = imagedata.split(" ")[0]
        if imagefile.endswith("\n"):
            imagefile = imagefile.replace('\n', '')
            continue
        imagename = os.path.basename(imagefile)
        jsonname = imagename.split(".")[0] + ".json"
        gauge_json = readJsonGetGauge(os.path.join(json_path,jsonname))
        #print("标签数据结果：")
        print(jsonname)
        for i in range(len(gauge_json)):
            flag = gauge_json[i].gaugeRead()
            if flag:
                read_data = gauge_json[i].gaugeread
                read_labels.update({encoding(gauge_json[i].center):read_data})
                c_range = get_range(gauge_json[i].gaugerange,gauge_json[i].scale)
                range_labels.update({encoding(gauge_json[i].center):c_range})
                count_read_sum += 1
        print(imagefile)
        image = Image.open(imagefile)
        image,center,pointer,scale,num_p,num_data = centernet.detect_image(image,args,crop=crop)
        ...
        for i in range(len(gauge_centernet)):
            ...
            if flag:
                center_encoding = read_labels.keys()
                print(center_encoding)
                center_labels = [decoding(p) for p in center_encoding]
                print(center_labels)
                center_model = gauge_centernet[i].center
                for c in center_labels:
                    if get_distence(c,center_model)<=100:
                        read_label = read_labels[encoding(c)]
                        range_ = range_labels[encoding(c)]
                        if abs(read_-read_label)<= 0.05*range_:
                            count_correct_005 += 1
                            count_correct_010 += 1
                        elif abs(read_-read_label)>= 0.05*range_ and abs(read_-read_label)<= 0.1*range_:
                            count_correct_010 += 1
        image.save(os.path.join(image_save_path,imagename),quality=95)
        #print(count_read_sum)
        #print(count_correct_005)
        #print(count_correct_010)
    
    #print(count_read_sum/)
    print(count_correct_005/count_read_sum)
    print(count_correct_010/count_read_sum)