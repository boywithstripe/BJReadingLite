...
from utils.utils_bbox import decode_bbox, postprocess,get_point
from utils.utils_reading import Gauge,readJsonGetGauge,centernetOutGetGauge
...
class Crnn(object):
    def __init__(self,args):
        ...
        self.MODEL.NUM_CLASSES = len(self.config.DATASET.ALPHABETS)
    ...
    def recognition(self,image,converter,device):
        h , w = img.shape
        # 保持原比例的同时对图片进行resize
...
class CenterNet(object):
    _defaults = {
        ...
        "model_path": 'logs/loss_2022_04_16_16_11_09/ep140-loss0.886-val_loss1.003.pth',
        "classes_path": 'data/data_452_sf6_all/bj_classes.txt',
        # --------------------------------------------------------------------------#
        #   用于选择所使用的模型的主干
    ...
    def generate(self):
        ...
        #   载入模型与权值
        # -------------------------------#
        assert self.backbone in ['resnet50', 'hourglass', 'resnet18']
    ...
    def detect_image(self, image, crop=False):
        ...
        with torch.no_grad():
            ...
            test_numpy = outputs[0].cpu().numpy()
            test_num = test_numpy[:,3,:,:]
            max_index = np.unravel_index(np.argmax(test_num, axis=None), test_num.shape)
            print(test_num[max_index])
            ...
            print(outputs[0].shape)
            #key_points = get_point(outputs[0],outputs[2],self.confidence,self.cuda)
            #xv_mask , yv_mask = key_points
            #print(xv_mask.shape)
            #print(yv_mask.shape)
...
if __name__ == '__main__':
    crop = False
    centernet = CenterNet()
    image = Image.open("test_11.jpg")
    r_image = centernet.detect_image(image, crop=crop)
    image_save = r_image.save("s.jpg")
    r_image.show()
    """
    while True:
